...
with open(inputFilePath) as fp:
    for line in fp:
        ...
        row['momAge'] = int({'99': None}.get(line[74:76], line[74:76]))
        row['momRace'] = {'1': 'White', '2': 'Black', '3': 'AIAN', '4': 'Asian', '5': 'NHOPI', '6': 'Multi'}.get(line[106])
        row['isMomMarried'] = {'1': 1, '2': 0}.get(line[119], None)
        row['momBMI'] = float({'99.9': None}.get(line[282:286], line[282:286]))
        row['momHeightInches'] = int({'99': None}.get(line[279:281], line[279:281]))
        row['momWeightGain'] = int({'99': None}.get(line[303:305], line[303:305]))
        row['momPrePregWeightPounds'] = int({'999': None}.get(line[291:294], line[291:294]))
        row['numYearsSincePeriod'] = int({'9999': None}.get(line[480:484], 2014-int(line[480:484])))
        row['isMomHSGrad'] = {'1': 0, '2': 0, '9': None}.get(line[123], 1)

        row['dadAge'] = int({'99': None}.get(line[146:148], line[146:148]))
        ...
        row['numMonthsSinceLastLiveBirth'] = int({'999':None, '888':None}.get(line[197:200], line[197:200]))
        row['hadPriorBirthAndDead'] = {'99': None, '00': 1}.get(line[172:174], 1)
        row['hadPriorBirthAndAlive'] = {'99': None, '00': 1}.get(line[170:172], 1)

        row['prenatalCareMonthBegan'] = int({'00':None, '99':None}.get(line[223:225], line[223:225]))
        row['numPrenatalVisits'] = int({'99':None}.get(line[237:239], line[237:239]))

        row['numDailyCigarettesPriorPreg'] = int({'99':None}.get(line[252:254], line[252:254]))
        row['numDailyCigarettesFirstTri'] = int({'99':None}.get(line[254:256], line[254:256]))
        row['numDailyCigarettesSecondTri'] = int({'99':None}.get(line[256:258], line[256:258]))
        row['numDailyCigarettesThirdTri'] = int({'99':None}.get(line[258:260], line[258:260]))
